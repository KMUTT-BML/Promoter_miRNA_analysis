...
parser.add_option("-o", "--output", dest="out_dir",
	...
	help="Directory of output file (required)")
options,args = parser.parse_args()

if options.default == 'thesis':
	miRNA_gff = "data/Mes_pri_miRNA_loci.gff3"
	gene_gff = "data/Mesculenta_147_v4.1.gene.gff3"
	genome = "data/Mesculenta_147_v4.1.fa"
else:
	...
	if not os.path.isfile(options.gene_gff):
		sys.exit("Not found gene_gff sequence : %r" % options.out_dir)
	gene_gff = options.gene_gff
	out_classify = options.out_dir
...
os.makedirs(os.path.dirname(out_intergenic_mi_file_name), exist_ok=True)
...
for miRNA in miRNAs.getTable():
	if(miRNA[2] == 'miRNA_primary_transcript'):
		...
		# Write classify file
		text = chromosome+'\t'+ref+'\t'+'miRNA_primary_transcript'+'\t'+str(start)+'\t'+str(stop)+'\t'+'.'+'\t'+stand+'\t'+'.'+'\t'+'ID='+ miRID + ';Alias=;Name='+miRName
		# print(text)
		if type == 'Intergenic':
			if(stand == '+'):
			...
			out_file_inter.write(text+'\n')
		elif type =='Intronic':
			if(stand == '+'):
				countIntronic_plus += 1
			else:
				countIntronic_minus += 1
			out_file_intro.write(text+'\n')
		elif type =='Exonic':
			if(stand == '+'):
				countExonic_plus += 1
			else:
				countExonic_minus += 1
			out_file_exo.write(text+'\n')
...
countIntergenic = countIntergenic_plus + countIntergenic_minus
countIntronic = countIntronic_plus + countIntronic_minus
countExonic = countExonic_plus + countExonic_minus

print("Number of intergenic miRNA ", countIntergenic, "[", countIntergenic_plus, "plus stand|", countIntergenic_minus, "minus stand]")
print("Number of introgenic miRNA ", countIntronic, "[", countIntronic_plus, "plus stand|", countIntronic_minus, "minus stand]")
print("Number of exonic miRNA     ", countExonic, "[", countExonic_plus, "plus stand|", countExonic_minus, "minus stand]")
